...
<head>
	...
	<script type="text/javascript"> 
		...
		// --------------------------- 消息推送start ---------------------------
		PL.userId='${fns:getUser().id}';  
	    // PL._init();
	    
	    // 添加主题
	    var subject = '/common';
	    var msgsubject = '${fns:getUser().msgSubject}';
	    if (msgsubject) {
	    	subject = subject + ',' + msgsubject;
	    }
	    PL.joinListen(subject);   // 接收点播时使用
	    ...
	    function onData(event) {
	    	...
	    	top.$.jBox.messager(
    			decodeURIComponent(event.get("msgContent")), 
    			decodeURIComponent(event.get("msgTitle")), null, 
    			{ width: 350, icon: 'info', showType: 'show', buttons: { '查看详情': true }, submit: submit}
	    	);
	    }  
		// --------------------------- 消息推送end ---------------------------

...
public class BaseDao<T> {
	 ...
	 * @return
	 */
	public Object uniqueResult(String qlString, Parameter parameter) {
		return createQuery(qlString, parameter).uniqueResult();
	}
	...
	public int deleteById(Serializable id, String likeParentIds) {
		if (isDelFlag())
			...
			return update("delete from " + entityClass.getSimpleName() + " where id = :p1 or parentIds like :p2",
	...
	private boolean isDelFlag() {
		if (containDelFlagClazzMap.containsKey(entityClass))
			return containDelFlagClazzMap.get(entityClass);
		...
		containDelFlagClazzMap.put(entityClass, isContain);
		return isContain;
	}

	private static Map<Class<?>, Boolean> containDelFlagClazzMap = new HashMap<Class<?>, Boolean>();

...
<body>
		...
		<c:forEach items="${page.list}" var="unitCatalog">
			<tr id="${unitCatalog.id}" pId="${unitCatalog.parent.id ne '0' ? unitCatalog.parent.id : '0'}">
				...
				<shiro:hasPermission name="asmt:unitCatalog:edit"><td>
					...
					<a href="${ctx}/asmt/unitCatalog/delete?id=${unitCatalog.id}" onclick="return confirmx('确认要删除该单位目录吗？', this.href)">${empty unitCatalog.parent  ? '':'删除'}</a>
					<a href="${ctx}/asmt/unitCatalog/form?parent.id=${unitCatalog.id}">添加下级菜单</a> 
	...
	<form:form id="submitForm" modelAttribute="unitCatalog" action="${ctx}/asmt/unitCatalog/submit" method="post">
